...
{
	menu(separator="after" title="复制路径" image=icon.copy_path)
	{
		item(where=sel.count > 1 title='复制已选 (@sel.count) 项' cmd=command.copy(sel(false, "\n")))
	...
	menu(separator="after" image=\uE290 title="选择")
	{
		...
		item(title="取消选择" image=icon.select_none cmd=command.select_none)
	}
	...
	menu(mode="single" type='back' expanded=true)
	{
		...
		{
			item(title='日期时间' cmd=io.dir.create(sys.datetime("ymdHMSs")))
	...
	item(where=!wnd.is_desktop title="文件夹选项" image=icon.folder_options cmd=command.folder_options)
}