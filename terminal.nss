menu(type='*' where=(sel.count or wnd.is_taskbar or wnd.is_edit) title=title.terminal sep='top' image=icon.run_with_powershell)
{
	...
	item(title='Ubuntu (WSL)' tip=tip_run_admin admin=has_admin image=image.res("shell32.dll", 3) cmd='C:\WINDOWS\system32\wsl.exe --distribution-id {eb24dd16-faaa-43b8-8990-04f47ac67d75}' args='-noexit -Command wsl.exe -d Ubuntu')
}
